...
import {
  initialiseTaskTable,
  taskNameUpdate,
} from "./helpers/boardTasksUpdateHelper";
                  ...
                  return (
                    <tr className={`table-row ${theme}`}>
                      ...
                      <td data-cell="name" className="name">
                        <div
                          className="task-name"
                          ref={(el) => (activeHeaderRef.current[index] = el)}
                          ...
                          {activeInput && activeInput?.index === index ? (
                            ...
                            >
                              ...
                              {openTask?.index === index ? (
                                <span
                                  onClick={(e) => handleTaskOpen(e, task?._id)}
                                  style={{
                                    padding: "2px",
                                    border: "1px solid lightgray",
                                  }}
                                >
                                  <>{iconsMap.expand()}&nbsp; Open</>
                                </span>
                              ) : null}
                            </div>
                          )}
                      ...
                      >
                        <div
                          className={`status-value ${task?.status
                            ?.trim()
                            .split(" ")
                            .join("-")
                            .toLowerCase()} ${theme}`}
                      ...
                      >
                        <div
                          className={`label-value ${task?.label
                            ?.trim()
                            .split(" ")
                            .join("-")
                            .toLowerCase()} ${theme}`}
