...
const useSideMenuContext = () => {
  ...
  if (!context) {
    throw new Error(
      "Children cannot be rendered outside Sidebar parent component"
    );
  }
...
const Sidebar = ({ children, showSideMenu }) => {
  ...
  return (
    <SideMenuContext.Provider
      value={{ activeIndex, setActiveIndex, panel, setPanel }}
    >
      <div
...
const MenuList = ({ children }) => {
  ...
  return <div className="sidemenu__list">{Children}</div>;
};
...
const MenuIndex = ({
  ...
  panelData,
  bottom = false,
  handleIndexAction = undefined,
  id,
  theme,
}) => {
  const [arrow, setArrow] = useState(false);
  const showArrow = () =>
    arrowOnHover ? (arrow ? setArrow(false) : setArrow(true)) : null;
  ...
  return (
    <div
      className={`menu__index sidebar-index ${
        theme === "light" ? "light_index" : "dark_index"
      } 
    ...
    >
      {children}
      <div>{arrow ? iconsMap.doubleRightArror(14) : null}</div>
    </div>
  );
};
...
const SidePanels = ({ children, id }) => {
  ...
  return (
    <div
      className={`sidemenu__sidePanels ${
        activeIndex?.index && activeIndex?.id === id && panel.show
          ? "show"
          : "hide"
      } sidebar`}
    >
...
const SidePanelIndex = ({ children, handleAction, options }) => {
  if (options) {
    return (
      <div
        className={`sidemenu__sidepanel sidebar-index`}
        onClick={() => handleAction(options?.id)}
      >
        <div className="sidepanel_index_tag-container">
          <Tag
            className={`sidepanel_index_status-tag ${options?.status
              .trim()
              .toLowerCase()
              .split(" ")
              .join("_")}`}
          ...
          <Tag
            className={`sidepanel_index_label-tag ${options?.label
              .trim()
              .toLowerCase()
              .split(" ")
              .join("_")
